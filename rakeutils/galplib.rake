...
DEPS = { "#{GALPDIRS[:galp] || '/home/linsj/MCGAL/galp'}": %w(galprop skymap),
         "#{GALPDIRS[:package] || 'MCGAL/package'}": { 'Healpix_2.20a/src/cxx/generic_gcc': %w(healpix_cxx cxxsupport fftpack),
                                                       'ccfits': %w(CCfits),
                                                       'CLHEP': %w(CLHEP),
                                                       'cfitsio': %w(cfitsio) },
         nil => %w(lins works),
